...
mock openai SDK（DeepSeek via OpenAI-compatible protocol），覆盖 4 个关键 case：
...
def _make_openai_mock(response_text: str):
    """构造 mock openai.OpenAI 类，调 chat.completions.create 返指定 text。"""
...
def test_markdown_fenced_json_accepted():
    ...
    fake = _make_openai_mock("""Here's my answer:
```json
...
def test_no_candidates_short_circuits():
    ...
    fake = _make_openai_mock("should not be called")
    with patch.dict(sys.modules, {"openai": fake}):

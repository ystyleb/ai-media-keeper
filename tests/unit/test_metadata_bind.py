...
def test_bind_movie_happy_writes_cache_with_manual_pick_source(client, token):
    ...
    with (
        patch.object(app_module, "_ssh_stat_paths", return_value=_stat_ok(path)),
        patch.object(app_module, "get_tmdb_provider", return_value=fake_provider),
    ):
        resp = client.post(
            "/api/metadata/bind",
...
def test_bind_lookup_raises_returns_502(client, token):
    ...
    with (
        patch.object(app_module, "_ssh_stat_paths", return_value=_stat_ok()),
        patch.object(app_module, "get_tmdb_provider", return_value=fake_provider),
    ):
        resp = client.post(
            "/api/metadata/bind",
...
def test_bind_rebind_overwrites_previous(client, token):
    ...
    matrix = type(
        "P", (), {"lookup_by_id": lambda self, *a, **kw: _make_movie_details(tmdb_id="603")}
    )()
    # 第 2 次绑到 Inception
    ...
    with app_module.app.app_context():
        rows = (
            app_module.get_db()
            .execute("SELECT title, tmdb_id FROM media_files WHERE path = ?", (path,))
            .fetchall()
        )
